...
Then wiht the command `:Select test` you can select a value from the list and
...
=== I want it to be available only for some filetypes
...
for an action. It receives a current working directory path (which is ignored
in this example) and a selected value.

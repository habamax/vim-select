...
* `:SelectFile` to navigate filesystem and select/open files and directories.
  ...
  argument.
* `:SelectBuffer` to select and open a buffer.
* `:SelectMRU` to select and open a file from history (most recently used).
* `:SelectCmd` to select and execute an `:ex` command without parameters.
* `:SelectColors` to select and apply a colorscheme.
